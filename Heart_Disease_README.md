...
![Dataset](https://img.shields.io/badge/Dataset-UCI%20Heart%20Disease-red?style=flat)
...
1. **Data loading & inspection** — shape, types, nulls, duplicates, data dictionary
2. **Exploratory data analysis (EDA)** — distributions, box plots, count plots, outlier detection
3. **Correlation & statistical analysis** — Pearson heatmap, chi-square tests, scatter plots
4. **Dashboard** — 6-panel visualisation exported as a PNG
...
1. **`oldpeak`** (−0.44) — ST depression is the single strongest predictor
2. **`exang`** (−0.44) — Exercise-induced angina nearly equals oldpeak in strength
3. **`cp`** (+0.43) — Chest pain type is the top positive predictor
...
![Heart Disease Dashboard](outputs/heart_disease_dashboard.png)
...
Download `heart.csv` from [Kaggle](https://www.kaggle.com/datasets/johnsmith88/heart-disease-dataset) and place it in the `data/` folder.
